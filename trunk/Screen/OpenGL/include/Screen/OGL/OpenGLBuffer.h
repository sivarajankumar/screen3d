...
namespace Screen {
	namespace OGL {
	    ...
	    public :
	    	...
	    	OpenGLBuffer(unsigned long count, unsigned int buffer)
	    		:BufferBase(count),buffer(buffer)
	    	{}
	        
	    	virtual ~OpenGLBuffer(){
	    		if (buffer)
	    			OpenGLRenderer::glDeleteBuffersARB(1, &buffer);
	    	}
	    ...
	    typedef OpenGLBuffer<GL_ELEMENT_ARRAY_BUFFER_ARB> OpenGLIndexBuffer;
	}
}
